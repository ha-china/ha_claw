...
_ACTIVITY_CONTEXT_BLOCK_RE = re.compile(
    r"<\s*activity-context\s*>[\s\S]{0,50000}?</\s*activity-context\s*>",
    flags=re.IGNORECASE,
)
_ACTIVITY_CONTEXT_TAG_RE = re.compile(r"</?\s*activity-context\s*>", flags=re.IGNORECASE)
...
def _strip_memory_context(text: str) -> str:
    ...
    text = _ACTIVITY_CONTEXT_BLOCK_RE.sub("", text)
    text = _ACTIVITY_CONTEXT_TAG_RE.sub("", text)
    return text
...
def get_response_text(result: Any, *, language: str | None = None) -> str:
    ...
    if not result or not result.response or not result.response.speech:
        return ""
    from ..output.reply_formatter import strip_reply_prefix
...
def apply_agent_response_format(
    ...
    *,
    ...
    from ..core.state import get_channel_type, get_conversation_status
    ...
    from ..output.reply_formatter import strip_reply_prefix, format_reply_speech, format_detailed_speech
