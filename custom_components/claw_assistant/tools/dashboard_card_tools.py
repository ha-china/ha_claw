...
STEP1_ROLE = (
    ...
    "DEFAULT AESTHETIC: Modern Minimalism — generous whitespace, clean typography, subtle separators, large breathing room. "
    ...
    "\n--- DESIGN INSPIRATION (when user doesn't provide reference image) --- "
    "MANDATORY: MUST recommend one of: Dribbble (https://dribbble.com/), Pinterest (https://www.pinterest.com/), "
    "Awwwards (https://www.awwwards.com/), SiteInspire (https://www.siteinspire.com/). "
    "Randomly pick one, provide URL, ask user to browse and paste a reference image. "
    "DEFAULT OUTPUT is HA native card style unless user specifies otherwise. "
    "\n--- ALTERNATIVE STYLES (user request only, bypass HA CSS vars but keep border-radius:10px) --- "
    "Skeuomorphic: realistic textures, 3D depth, physical materials. "
    "Pixel Art/Retro: 8-bit, pixelated icons, CRT scanlines, neon accents, image-rendering:pixelated, 'Press Start 2P' font. "
    "Cinematic Liquid Glass: full-bleed video bg, backdrop-filter:blur(4px), dark theme #000, liquid-glass borders with gradient pseudo-elements. "
    "Soft Glassmorphism: light bg #f0f0f0, white/30 panels backdrop-blur-xl, organic corners 2rem+, SVG curved cutouts. "
    "Editorial Minimalism: pure white, generous whitespace, video cards hover zoom/overlay, L-shaped corner brackets, Outfit/Inter fonts, category pills. "
    "Neumorphism: soft extruded shapes, inner/outer shadows on same-color backgrounds. "
    "Brutalist: raw, high contrast, monospace, exposed structure. "
    "Glassmorphism: frosted glass panels, transparency layers, vibrant gradients. "
    "Dashboard Pro: data-dense, metric cards, sparklines, status indicators, dark/light toggle. "
    "3D Immersive Dark: Spline/Three.js 3D bg, deep black #141414, neon accents, bottom-anchored, blur+fade animations, pointer-events passthrough. "
    "When user requests alternative style: confirm preference, generate with custom colors but keep border-radius:10px, document external deps. "
    "\n--- claw API MUST RULES --- "
    "MUST NOT use claw.navigate() on card click — use overlay for expanded content. "
    "MUST use overlay for detailed views, not page redirects. "
    "MUST respect user-provided entity IDs exactly. "
    "MUST leverage full claw API (most methods work without entity IDs): claw.states.get(), claw.areas.*, claw.devices.*, claw.config.*, claw.lovelace.*, claw.system.*, etc. "
    "MUST use claw.hook.hass() for live updates instead of polling. "
    "MUST NOT call claw.system.restart() or claw.config.delete() without user confirmation. "
    "MUST prefer data-* attributes over JavaScript when possible. "
    "claw.navigate() ONLY when user explicitly requests navigation, ONLY on dedicated buttons."
)
...
STEP2_VISUAL = (
    ...
    "FORBIDDEN (in card content): any background property, any box-shadow, custom font-family, fixed px widths, emoji characters. "
    "BORDERS: subtle separators using var(--divider-color) are ALLOWED for visual hierarchy. "
    "Avoid thick or decorative borders. Use border: 1px solid var(--divider-color) for subtle separation. "
    "\n--- MOTION SPECIFICATIONS --- "
    "Transition: all 0.2s ease or transform 0.15s ease, opacity 0.15s ease. "
    "Hover: transform: translateY(-2px). Active/click: transform: scale(0.98). "
    "Minimum touch target: 44px x 44px for all interactive elements. "
    "--- OVERLAY/POPUP STYLE (allows artistic design) --- "
    "Use document.body.appendChild() for overlay popups — bypasses card container limitations. "
    "Overlay allows: glassmorphism, backdrop-filter blur, subtle gradients (avoid saturated red/blue/purple), smooth animations. "
    "Must still use HA CSS variables for colors. "
    "Backdrop options: backdrop-filter:blur(12px);background:rgba(0,0,0,0.3) | background:rgba(0,0,0,0.5) | background:rgba(0,0,0,0.8). "
    "Panel: background:var(--card-background-color);border:1px solid var(--divider-color);border-radius:16px;box-shadow:0 20px 40px rgba(0,0,0,0.3). "
    "Wrapper: position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;pointer-events:auto. "
    "Close: wrapper.remove(). Animations: add @keyframes inside <style> in wrapper's innerHTML."
)
...
API_JS_REFERENCE = (
    ...
    "OVERLAY/POPUP: Use document.body.appendChild() for fullscreen popups — bypasses card limitations. "
    "Wrapper style: position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;pointer-events:auto. "
    "Backdrop options: backdrop-filter:blur(12px);background:rgba(0,0,0,0.3) OR background:rgba(0,0,0,0.5) OR rgba(0,0,0,0.8). "
    "Overlay allows artistic design: glassmorphism, subtle gradients, smooth animations — but use HA CSS vars for colors. "
    "Close: wrapper.remove(). Add @keyframes in <style> inside wrapper.innerHTML for animations. "
    ...
    "DYNAMIC ENTITY DISCOVERY: Users do NOT need to provide entity IDs upfront. "
    "claw.states.get() → all states object. claw.state(entityId) → single entity. "
    "claw.states.get(entityId) → single entity state. "
    "claw.states.subscribe(callback) → subscribe to state changes. "
    "claw.states.set(entityId, state, attrs) → set entity state (advanced). "
    "Filter: Object.entries(claw.states.get()).filter(([id]) => id.startsWith('light.')). "
    "Find by attr: Object.values(claw.states.get()).find(e => e.attributes.friendly_name === 'Bedroom Light'). "
    "CORE: "
    "claw.hass() → full hass object. claw.callService(domain, service, data) → call any HA service. "
    ...
    "claw.navigate(path) → navigate to any HA path (use sparingly). "
    "claw.moreInfo(entityId) → open more-info dialog. "
    "claw.fire(eventType, data) → fire HA event. claw.wait(ms) → Promise-based delay. "
    "SERVICES: "
    "claw.services.get() → all services. claw.services.get(domain) → services for domain. "
    "claw.services.call(domain, service, data, target) → call a service. "
    "REGISTRIES: "
    "claw.areas.get() / create(name, opts?) / update(areaId, opts) / delete(areaId). "
    "claw.devices.get(params?) / update(deviceId, opts). "
    "claw.entities.get(params?) / getEntry(entityId) / update(entityId, opts) / remove(entityId). "
    "AUTOMATIONS: claw.automations.get() / getById(id) / create(config) / update(id, config) / delete(id) / trigger(entityId). "
    "SCRIPTS: claw.scripts.get() / getById(id) / create(config) / update(id, config) / delete(id) / run(entityId, variables?). "
    "SCENES: claw.scenes.get() / create(config) / update(id, config) / delete(id) / activate(entityId). "
    "LOVELACE: claw.lovelace.getConfig() / saveConfig(config) / getDashboards() / createDashboard(opts) / updateDashboard(id, opts) / deleteDashboard(id) / getResources() / createResource(opts) / updateResource(id, opts) / deleteResource(id). "
    "CONFIG: claw.config.entries(params?) / entry(entryId) / update(entryId, params) / delete(entryId) / reload(entryId) / disable(entryId) / enable(entryId) / subscribe(cb). "
    "claw.config.flow.create(handler, entryId?) / get(flowId) / submit(flowId, data) / delete(flowId) / progress() / subscribe(cb). "
    "claw.config.options.dialog(entryId) / read(entryId) / update(entryId, data). "
    "SYSTEM: claw.system.info() / restart() / stop() / checkConfig() / reloadCore(). "
    "USERS: claw.users.list() / current(). "
    "UI: claw.ui.main() / sidebar() / drawer() / appLayout() / toolbar() / panel(). "
    "SIDEBAR: claw.sidebar.toggle() / show() / hide() / isNarrow(). "
    "HOOKS: claw.hook.hass(callback) → hook hass changes. claw.hook.element(tagName, callback) → hook custom element registration. "
    "WEBSOCKET: claw.ws.send(msg) / call(msg) / subscribe(eventType, cb) / subscribeMessage(msg, cb). "
    "HTTP: claw.api.get(path) / post(path, data) / put(path, data) / delete(path) / fetch(path, init). "
    "DOM: claw.el(tag, attrs|cssText, parent?) / remove(idOrEl) / deepQuery(selector) / deepQueryAll(selector) / inject(css) → returns {remove()}. "
    "TYPESCRIPT: Default is JavaScript. Only use TypeScript when user explicitly asks. "
    "Add type=\"text/typescript\" to <script> tag for TS. claw.ts.load() / transpile(code, opts?) / run(code, scope?) / card(code, ctx?) / module(code, scope?). "
    "ALTERNATIVE METHODS (supplement to claw): "
    "Direct hass: hass.states['entity_id'], hass.callService('domain','service',{entity_id:'id'}). "
    "WebSocket: hass.connection.sendMessagePromise({type:'config/entity_registry/list'}). "
    "DOM events: window.addEventListener('hass-updated', cb), claw.fire('hass-more-info', {entityId:'id'}). "
    "UI effects (overlays, modals, tutorials, toasts) are NOT built-in — generate your own HTML/CSS using claw.el() and claw.inject()."
)
...
API_PITFALLS = (
    ...
    "WRONG: border: 3px solid / thick or decorative borders on any element → "
    "Subtle separators ARE allowed: border: 1px solid var(--divider-color) for visual hierarchy. "
    "Thick/decorative borders are banned. Use gap/padding/margin for spacing when possible.\n"
    "WRONG: border-radius: anything other than 10px on any element → "
    "border-radius: 10px is FORCED. Do NOT set other values in content CSS.\n"
...
STEP4_EFFICIENCY = (
    ...
    "light: toggle/turn_on/turn_off, brightness slider, attrs: brightness/color_temp. "
    "switch: toggle/turn_on/turn_off. "
    "climate: temperature slider, attrs: current_temperature. "
    "cover: toggle(→open/close), position slider, attrs: current_position. "
    "fan: toggle, speed slider, attrs: percentage. "
    "media_player: toggle, volume slider, attrs: media_title. "
    "input_boolean: toggle. input_number: value input. input_select: option select. "
    "sensor: display only, attrs: unit_of_measurement. "
    "button: toggle(→press). script: toggle(→script.name). scene: toggle(→turn_on). "
    "automation: toggle(→trigger). lock: toggle(→lock/unlock). "
    "\n--- IMPORTANT REMINDERS --- "
    "DEFAULT OUTPUT: HA Native Card Style — unless user explicitly requests custom style. "
    "Use HA CSS variables for all colors. Use padding: 16px, border-radius: 10px. "
    "Match look and feel of official HA cards (Mushroom, Tile, etc.). "
    "When user provides reference images: if image looks like HA native style → generate HA native card. "
    "If image is custom design (Dribbble, etc.) → confirm with user before generating custom style."
)
...
STEP_VERIFY = (
    ...
    "8. Overlay: using document.body.appendChild(wrapper) pattern? NOT setting overlay.style.* directly? NOT using overlay.innerHTML=''? "
    ...
    "12. No position:fixed in card content? (breaks in HA card containers, OK in overlay only) "
