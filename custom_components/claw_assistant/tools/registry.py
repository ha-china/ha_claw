...
TOOL_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "ConversationMemory": {"category": "misc", "desc": "[Self Memory] Simple key-value store for user preferences and short facts. Auto-injected into system prompt. Do NOT use together with MemoryGraph for the same fact. Params: action (save/get/list/clear), target (memory/user), key, value", "priority": 3},
    "MemoryGraph": {"category": "core", "desc": "[Knowledge Graph] Relational graph memory (SQLite + FTS5, BM25, typed edges). Use for decisions, bug fixes, causal links needing graph traversal. Do NOT use together with ConversationMemory for the same fact. Params: action(recall/remember/link/pin/forget/get/stats/cleanup). recall: query, kinds?, limit?, expand?. remember: kind, title, body, source_doc?, confidence?, pinned?. link: src_id, dst_id, relation, weight?. pin: id, pinned?. forget: id. get: id. stats: none. cleanup: none (dedup + remove junk + rebuild edges).", "priority": 1},
    "ParallelToolCall": {"category": "misc", "desc": "ALWAYS use when 2+ independent tool calls exist. True parallel execution (asyncio.gather). Params: tools=[{name,args}]. Works with HAControl, EntityQuery, HistoryQuery, WebSearch, etc. Example: [{\"name\":\"HAControl\",\"args\":{\"action\":\"shell\",\"params\":{\"command\":\"cmd1\"}}},{\"name\":\"HAControl\",\"args\":{\"action\":\"shell\",\"params\":{\"command\":\"cmd2\"}}}]", "priority": 3},
