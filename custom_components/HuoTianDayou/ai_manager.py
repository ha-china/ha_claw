...
from .prompt_manager import PromptManager, QueryType
...
class AIManager:
    ...
    def __init__(self, hass: HomeAssistant):
        ...
        self.search_keywords = {
            ...
            'search_actions': [r'百度一下','搜一下','查一下','搜索', r'查找', r'查询', r'了解', r'联网', r'网上'],
        ...
        while True:
            try:
                ...
                for conv_id in expired_contexts:
                    ...
                    _LOGGER.info(f"Cleaned up expired context for conversation {conv_id}")
                
                await asyncio.sleep(300)  
            except Exception as e:
                _LOGGER.ino(f"Error in context cleanup: {e}")
    ...
    async def process_query(self, 
        ...
        if not agents:
            _LOGGER.error("未配置任何对话代理，无法处理请求")
            return self._create_error_response(user_input, "请配置至少一个对话代理。")
        
        _LOGGER.info(f"开始处理用户查询: '{user_input.text}'")
        ...
        if web_search_enabled:
            if self._has_url(user_input.text):
                ...
                _LOGGER.info("检测到URL，将直接获取内容")
            else:
                should_search, search_engine = self._should_perform_search(user_input.text)
                if should_search:
                    _LOGGER.info(f"检测到搜索关键词，使用搜索引擎: {search_engine}")
                elif web_search_results:
                    _LOGGER.info("未检测到搜索关键词，但收到搜索结果，将忽略搜索结果")
                    web_search_results = None
        elif web_search_results:
            _LOGGER.info("网络搜索功能已禁用，忽略搜索结果")
            web_search_results = None
            ...
            if context:
                ...
                if is_navigation and 'all_results' in context['data']:
                    ...
                    if result:
                        ...
                        _LOGGER.info(f"切换到搜索结果 {new_index + 1}/{len(all_results)}")
                else:
        ...
        if web_search_results and (should_search or is_context_dependent):
            ...
            _LOGGER.info(f"处理了网络搜索结果，类型: {query_type.value}, 处理后长度: {len(processed_search_results) if processed_search_results else 0}")
        ...
        _LOGGER.info(f"生成了动态提示词, 查询类型: {prompt_data['query_type']}, 内容类型: {prompt_data['content_types']}")
        
        if not isinstance(prompt_data['main_prompt'], str):
            _LOGGER.info("提示词不是字符串类型，尝试转换")
            try:
                ...
                _LOGGER.info("提示词已转换为字符串")
            except Exception as e:
                _LOGGER.error(f"转换提示词失败: {e}")
    ...
    async def _process_with_agents(self,
        ...
        for agent_id in agents:
            ...
            _LOGGER.info(f"使用代理 '{agent_name}' (ID: {agent_id}) 处理查询")
            ...
            try:
                ...
                _LOGGER.info(f"代理 '{agent_name}' 未能生成有效响应")
                continue
                
            except Exception as e:
                _LOGGER.error(f"使用代理 '{agent_name}' 处理查询时出错: {e}", exc_info=True)
                continue
    ...
    async def _async_process_agent(self,
                                ...
                                user_input: conversation.ConversationInput,
                                conversation_mode: str,
                                previous_result: Optional[Any]) -> conversation.ConversationResult:
