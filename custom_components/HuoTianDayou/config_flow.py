...
from .const import (
    ...
    CONF_ENABLE_WEB_SEARCH, CONF_WEB_SEARCH_ENGINE, SEARCH_ENGINE_GOOGLE, SEARCH_ENGINE_BAIDU, 
    SEARCH_ENGINE_BING, DEFAULT_SEARCH_ENGINE,
...
def get_conversation_agents(hass: HomeAssistant) -> list[dict[str, str]]:
    ...
    for entity_id in hass.states.async_entity_ids("conversation"):
        try:
            ...
            if state and state.attributes.get("entity") != "HuoTianDaYou.ai":
                agents.append({
                    "value": entity_id,
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_conversation_settings(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        current_enable_web_search = self._config_entry.options.get(CONF_ENABLE_WEB_SEARCH, False)
        current_search_engine = self._config_entry.options.get(CONF_WEB_SEARCH_ENGINE, DEFAULT_SEARCH_ENGINE)
        ...
        if current_enable_web_search:
            schema = schema.extend({
                vol.Optional(CONF_WEB_SEARCH_ENGINE, default=current_search_engine, description={"suggested_value": current_search_engine}): SelectSelector(
                    SelectSelectorConfig(
                        options=[
                            {"value": SEARCH_ENGINE_BAIDU, "label": "百度"},
                            {"value": SEARCH_ENGINE_GOOGLE, "label": "谷歌"},
                            {"value": SEARCH_ENGINE_BING, "label": "必应"}
                        ],
                        mode=SelectSelectorMode.DROPDOWN
                    )
                ),
